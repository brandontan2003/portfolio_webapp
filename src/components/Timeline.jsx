...
const timelineData = [
  {
    ...
    icon: <Briefcase className="w-4 h-4 md:w-5 md:h-5 text-secondary" />,
    color: 'secondary'
  },
  {
    id: 2,
    ...
    icon: <Briefcase className="w-4 h-4 md:w-5 md:h-5 text-primary" />,
    color: 'primary'
  },
  {
    id: 3,
    ...
    icon: <GraduationCap className="w-4 h-4 md:w-5 md:h-5 text-primary" />,
    color: 'primary'
  }
];
...
export default function Timeline() {
  return (
    <div className="w-full max-w-4xl mx-auto mb-12 sm:mb-20 relative px-4 md:px-0">
      ...
      <div className="relative pl-0">
        ...
        <div className="absolute left-5 md:left-1/2 top-0 bottom-0 w-[2px] bg-primary/20 transform -translate-x-1/2 md:-translate-x-1/2 shadow-[0_0_10px_rgba(34,211,238,0.5)]"></div>
        ...
        {timelineData.map((item, index) => (
          <motion.div 
            ...
            className={`relative mb-8 sm:mb-12 md:mb-24 flex md:justify-between items-center w-full ${
              index % 2 === 0 ? 'md:flex-row-reverse' : ''
            }`}
          >
            {/* Center Node */}
            <div className="absolute left-5 md:left-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-background transform -translate-x-1/2 flex items-center justify-center z-10 bg-slate-900 shadow-[0_0_15px_rgba(34,211,238,0.8)]">
              {item.icon}
            </div>

            {/* Content Box */}
            <div className="w-[calc(100%-3rem)] md:w-[45%] ml-12 md:ml-0 relative group">
              {/* Connector line for desktop */}
              <div className={`hidden md:block absolute top-5 w-8 h-[2px] bg-primary/40 ${
                index % 2 === 0 ? '-left-8' : '-right-8'
              }`}></div>
              
              <div className={`glass-panel p-4 sm:p-6 relative overflow-hidden ${item.color === 'secondary' ? 'glass-panel-purple' : 'glass-panel-hover'}`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                  <h3 className={`text-xl font-bold ${item.color === 'secondary' ? 'neon-text-secondary' : 'neon-text-primary'}`}>
                    {item.title}
                  </h3>
                  <span className="text-xs text-slate-400 font-mono bg-slate-900/50 px-2 py-1 rounded w-fit shrink-0">
                    {item.period}
                  </span>
                </div>
